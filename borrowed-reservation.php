...
<?php 
	...
	if (isset($_GET['action']) && $_GET['action'] == 1) {
		...
		if (isset($_GET['request_no'])) {
			...
			// For example:
			$query = "UPDATE request SET status = \"returned\" , request_date = NOW()  WHERE request_no = $request_no";
	...
	if ($users) {
		while ($user = mysqli_fetch_assoc($users)) {
			...
			$user_list .= "<td>";
			$user_list .="<a href='borrowed-reservation.php?action=1&request_no={$user['request_no']}&equip_id={$user['euip_id']}'>Return</a>";
			$user_list .="</td>";
			$user_list .= "</tr>";
		}
	} else {
